...
CREATE TABLE ESA.EXTERNAL_PROJECT_NUMBER 
(
  ID NUMBER NOT NULL 
...
, DATA_STEWARD_GROUP_ESA_ROLE_ID NUMBER 
...
, VENDOR_CATALOG_NUMBER VARCHAR2(256) 
...
, DOCUMENT_REFERENCE_URL VARCHAR2(4000) 
, DATE_CREATED DATE NOT NULL 
, CREATED_BY VARCHAR2(40) NOT NULL 
, LAST_UPDATED DATE NOT NULL 
, UPDATED_BY VARCHAR2(40) NOT NULL 
...
CREATE TABLE ESA.PROJECT_ALIAS 
(
  ID NUMBER NOT NULL 
...
CREATE TABLE ESA.PROJECT_ARCHITECT 
(
  ID NUMBER NOT NULL 
, PARTY_ID NUMBER NOT NULL 
, PROJECT_ID NUMBER NOT NULL 
...
ALTER TABLE ESA.EXTERNAL_PROJECT_NUMBER
ADD CONSTRAINT EXTERNAL_PROJECT_NUMBER_FK FOREIGN KEY
(
  PROJECT_ID 
)
REFERENCES ESA.PROJECT
(
  ID 
)
ENABLE;
...
ALTER TABLE ESA.PROJECT_ARCHITECT
...
COMMENT ON COLUMN ESA.ESA_USER.PARTY_ID IS 'The person associated with the user account.';

COMMENT ON COLUMN ESA.EXTERNAL_PROJECT_NUMBER.SOURCE_SYSTEM IS 'The source system identifying where the external project number is defined.  This is populated using a lookup value.';
...
COMMENT ON COLUMN ESA.ITEM.DATA_STEWARD_GROUP_ESA_ROLE_ID IS 'This is the esa_role.id responsible for the managment of the the item information.';
...
COMMENT ON COLUMN ESA.NOTE.DOCUMENT_REFERENCE_URL IS 'This is to proide a url reference to a document to support the note.';

COMMENT ON COLUMN ESA.PARTY_ADDRESS.PARTY_ID IS '@parties';

COMMENT ON COLUMN ESA.PARTY_ADDRESS.ADDRESS_ID IS '@addresses';

COMMENT ON COLUMN ESA.PROJECT.TYPE IS 'The type of project.  this value is populated using the project type lookup_list';

COMMENT ON COLUMN ESA.PROJECT.STATUS IS 'defines the status of the project such as OPEN, CLOSED, HOLD, CANCEL and so on.  This value should be populated using the project status lookup list.';

COMMENT ON COLUMN ESA.PROJECT.EXTERNAL_PROJECT_NUMBER IS 'the project reference number in the external project managment system';

COMMENT ON COLUMN ESA.PROJECT.PROJECT_MANAGER_PARTY_ID IS 'Reference to the project manager in the party table.  The party should be populated with a someone who has an enterprise architect role.';

COMMENT ON COLUMN ESA.PROJECT.DATE_START IS 'The date the project is started.';

COMMENT ON COLUMN ESA.PROJECT.DATE_COMPLETED IS 'The date the project was completed, closed or terminated.';
...
CREATE SEQUENCE ESA.EXTERNAL_PROJECT_NUMBER_SEQ INCREMENT BY 1 START WITH 1 NOCACHE;
...
CREATE SEQUENCE ESA.PROJECT_ALIAS_SEQ NOCACHE;

PROMPT create sequence 'PROJECT_ARCHITECT_SEQ'
CREATE SEQUENCE ESA.PROJECT_ARCHITECT_SEQ INCREMENT BY 1 START WITH 1 NOCACHE;

PROMPT create sequence 'PROJECT_SEQ'
CREATE SEQUENCE ESA.PROJECT_SEQ INCREMENT BY 1 START WITH 1 NOCACHE;
